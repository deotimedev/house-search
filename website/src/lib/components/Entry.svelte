...
<div class="w-[60vw] min-w-[300px] max-w-[95vw]">
    ...
    <div id="textbox" class="flex items-center bg-white rounded-2xl pt-1 pb-1">
        <div class="pl-[2vw] flex flex-col items-center bg-white rounded-2xl min-w-[50px]">
            <p class="font-bold text-[max(8px,1vw)]">{character.toUpperCase()}</p>
            <CharacterIcon {character} class="h-[3.5vw] min-w-[40px] min-h-[40px]" />
            <p class="text-[max(8px,1vw)]">S{entry.ep.season} E{entry.ep.number}</p>
        </div>
        <p class="pl-[2vw] flex-1 text-[max(12px,1.5vw)]">"{text}"</p>
    </div>
</div>